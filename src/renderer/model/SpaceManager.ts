...
export class SpaceManager{
    ...
    switchToUser(userId: any, dispatch: any,  workspaceState: any, sessionState: any, onComplete: any){
      UsersService.get(userId).then((user: any) => {
        WorkspaceService.getActiveWorkspacesByUserId(userId).then((workspaces) => {
          ...
          if(workspaces.length === 0){
            ...
            ProfilesService.getAllByUserId(userId).then((profiles) => {
              ...
              WorkspaceService.newWorkspace("Home", true, false, true, _profileId, userId).then((workspaceId) => {
                // Store home workspace ID in users table
                UsersService.setHomeWorkspace(userId, workspaceId);
                ...
                WorkspaceService.getActiveWorkspacesByUserId(userId).then((workspaces) => {
                  ...
                  dispatch(userActions.setHomeWorkspace({ id: workspaceId }));
                }).catch((error) => {
                ...
                DesktopService.newDesktop("Desktop-1", workspaceId, true).then((desktop) => {
                  WorkspaceService.selectWorkspaceById(dispatch, workspaceId, workspaceState, sessionState).then((workspace) => {
                    log.debug("Home workspace is ready...");
            ...
            // Check if home workspace exists, if not create one
            if(!user.homeWorkspace){
              ProfilesService.getAllByUserId(userId).then((profiles) => {
                let _profileId = profiles[0].id;
                // @ts-expect-error TS(2554): Expected 7-8 arguments, but got 6.
                WorkspaceService.newWorkspace("Home", true, false, true, _profileId, userId).then((homeWorkspaceId) => {
                  UsersService.setHomeWorkspace(userId, homeWorkspaceId);
                  dispatch(userActions.setHomeWorkspace({ id: homeWorkspaceId }));
                  ...
                  DesktopService.newDesktop("Desktop-1", homeWorkspaceId, true).then(() => {
                    log.debug("Home workspace created...");
                  });
                });
              });
            } else {
              dispatch(userActions.setHomeWorkspace({ id: user.homeWorkspace }));
            }
